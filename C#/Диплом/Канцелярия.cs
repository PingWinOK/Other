...
{
    public partial class Канцелярия : Form
    {
        public Канцелярия()
        {
            InitializeComponent();
        ...
        public void Обновление(DataGridView t)
        {
            ...
            Connect.Close();
            Connect.Open();
            string query = "SELECT * FROM diplom.tovari;";
        ...
        private void button7_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < dataGridView1.Rows.Count-1; i++)
            {
                ...
                Connect.Close();
                Connect.Open();
                fard = "UPDATE Tovari SET Товар ='" + dataGridView1.Rows[i].Cells[1].Value + "',Количество ='" + dataGridView1.Rows[i].Cells[2].Value + "',Цена =" + dataGridView1.Rows[i].Cells[3].Value + ",Сумма =" + dataGridView1.Rows[i].Cells[4].Value + ",Примечание = '" + dataGridView1.Rows[i].Cells[5].Value + "',Поставщик = '" + dataGridView1.Rows[i].Cells[6].Value + "' WHERE № =" + dataGridView1.Rows[i].Cells[0].Value;
        ...
        {
            Обновление(dataGridView1);
            conteyner.h = dataGridView1.RowCount + 1;
            ДобавлениеPos af = new ДобавлениеPos();
            af.Owner = this;
            af.ShowDialog();
            ...
            try
            {
                switch (comboBox1.Text)
                {
                    case "Найти":
                        ...
                        string query = "SELECT * FROM tovari WHERE " + comboBox2.Text + " LIKE '%" + textBox1.Text + "%'";
                        MySqlCommand command = new MySqlCommand(query, Connect);
                        MySqlDataReader Otvet = command.ExecuteReader();
                    ...
                    case "Максимальное число":
                        ...
                        textBox1.Text = "";
                        dataGridView1.Rows.Clear();
                        string query1 = "SELECT * FROM tovari WHERE " + comboBox2.Text + " =(SELECT MAX(" + comboBox2.Text + ") FROM Tovari)";
                    ...
                    case "Минимальное число":
                        ...
                        textBox1.Text = "";
                        dataGridView1.Rows.Clear();
                        string query2 = "SELECT * FROM tovari WHERE " + comboBox2.Text + " =(SELECT MIN(" + comboBox2.Text + ") FROM Tovari)";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dr == DialogResult.OK)
                {
                    ...
                    string query = "DELETE FROM tovari WHERE № = " + mas;
                    MySqlCommand command = new MySqlCommand(query, Connect);
                    MySqlDataReader Otvet = command.ExecuteReader();
        ...
        private void comboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            switch (comboBox1.Text)
            {
                case "Найти":
                    ...
                    string[] Найти = { "№", "Товар", "Количество", "Цена", "Сумма", "Примечание" , "Поставщик" };
        ...
        void SaveTable(DataGridView What_save)
        {
            ...
            string path = System.IO.Directory.GetCurrentDirectory() + @"\" + "Отчет по таблице Канцтовары(" + d.ToShortDateString() + " " + d.Hour + "." + d.Minute + "." + d.Second + ").xlsx";
            ...
            Excel.Range range1 = (Excel.Range)worksheet.Range[worksheet.Cells[2, 1], worksheet.Cells[2, 7]];
            // Выделение области под заголовки .Даем название range1
            Excel.Range range2 = (Excel.Range)worksheet.Range[worksheet.Cells[1, 1], worksheet.Cells[1, 7]];
            // Выделение области под надпись "Отчет бла бла бла" .Даем название range2
            worksheet.Cells[1, 1] = "Отчет по таблице Канцтовары "+ d.ToShortDateString();
