...
void __fastcall TForm1::N2Click(TObject *Sender)
{
    ...
    Label1->Caption="Размещение "+OpenDialog1->FileName;;
 ...
 //Сканирование файла на наличие непрерывной цепочки нулей
              if (hex=="00")
                  {
                   shet++;
                   if (shet>shetmax)
                       {
                        shetmax=shet;//Сохранение длины наибольшей цепочки
                        i1=i;//Запоминание позиции последнего байта цепочки
                        j1=j;
                       }
                  }
              else
...
//---------------------------------------------------------------------------
// Сохранение файла, содержащего ЦВЗ
...
//---------------------------------------------------------------------------
//Вызов формы ввода пароля для встраивания ЦВЗ
void __fastcall TForm1::N6Click(TObject *Sender)
{
    Form2->Caption="Встраивание ЦВЗ";
    Form2->Button1->Caption="Встроить";
    Form2->ShowModal();
    if ( ! Form2->isOK )
        {
         return;
        }
//Сравнение размеров ЦВЗ для встраивания и контейнера
    ifstream size_file(pname.c_str(), ios::in|ios::ate);
    if (size_file.tellg()>(shetmax-48))
        {
         if ((shetmax-48)>0)
             {
              ShowMessage("Размер встраиваемого изображения больше размера контейнера!");
              ShowMessage("Максимальный размер встраиваемого ЦВЗ: " +IntToStr(shetmax-48)+" байт!");
             }
         else
         ShowMessage("ЦВЗ встроить невозможно, недостаточно места!");
        }
    else
//Передача управления подпрограмме встраивания ЦВЗ
        {
         Insert(Sender);
         ShowMessage("ЦВЗ успешно встроен!");
...
{
    if ( MessageBox(NULL,"Вы действительно хотите закрыть приложение?","Внимание",MB_YESNO)== mrYes)
    Form1->Close();
}
//---------------------------------------------------------------------------
//Вызов формы пароля для извлечения ЦВЗ
void __fastcall TForm1::N8Click(TObject *Sender)
{
    Form2->Caption="Извлечение ЦВЗ";
    Form2->Button1->Caption="Извлечь";
    Form2->ShowModal();
    if ( ! Form2->isOK )
       {
        return;
       }
//Передача управления подпрограмме извлечения ЦВЗ
    Retrieve(Sender);
}
//---------------------------------------------------------------------------
//Подпрограмма внедрения ЦВЗ
void __fastcall TForm1::Insert(TObject *Sender)
{
    ifstream input_file(pname.c_str(), ios:: binary);
    int letter;
    int j=j2; //Присваивание j начальной строки встраивания ЦВЗ
    int i;
    char letter2;
    char* hashpass;
    j++;
//Встраивание хеш-значения пароля
    for (int i=1; i<=8; i++)
        {
...
//---------------------------------------------------------------------------
//Выделение цепочки, в которую будет произведено встраивание
//Вычисление началоной строки встраивания ЦВЗ
void __fastcall TForm1::StringGrid1DrawCell(TObject *Sender, int ACol,
      int ARow, TRect &Rect, TGridDrawState State)
{
//Получение местонахождения позиции последнего байта цепочки встраивания
    j2 = j1;
    i2 = i1;
    int prov=shetmax;
    while (prov>0)
         ...
         if (i2<1)
             {
              i2=16;
              j2--;
              if(ACol == i2 && ARow == j2) // определяем конкретную ячейку
                  {
...
void __fastcall TForm1::Retrieve(TObject *Sender)
{
    ...
    int jj;
    char* hashpass;
//Сравнение хэш-значения пароля, введённого пользователем с имеющимся в ехе-файле
 ...
 //Извлечение ЦВЗ
        {
         ofstream output_file("ЦВЗ.bmp", ios:: binary);
         ...
         output_file.close ();
         Form1->Image1->Picture->LoadFromFile("ЦВЗ.bmp");
         DeleteFile("ЦВЗ.bmp");
         ShowMessage("ЦВЗ успешно извлечён!");
        }
    else
    ShowMessage("ЦВЗ не встроен!");
}
//---------------------------------------------------------------------------
...
{
    WinExec(".\\Filigrana_Справка.chm",SW_SHOW);
    ShellExecute(0, "Open", ".\\Filigrana_Справка.chm" , "", "", 1);
}
//---------------------------------------------------------------------------
